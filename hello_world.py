...
g_window = pygame.display.set_mode((800, 600))  # 窗口大小
...
class Player(EntityLike):  # 玩家类
    ...
    def keydown(self):
        ...
        if keys[pygame.K_e] and not self.is_jumping:  # 跳跃只在未跳跃状态下触发
            self.is_jumping = True
            self.jump_velocity = -15  # 设置跳跃初速度（负值向上）

        # 跳跃逻辑
        if self.is_jumping:
            ny += self.jump_velocity
            self.jump_velocity += 10  # 模拟重力，使角色下降

            # 模拟着地检测（假设地面为窗口高度的500像素）
            if ny >= 500:
                ny = 500
                self.is_jumping = False  # 着地后停止跳跃
        
        if keys[pygame.K_ESCAPE]:  # 按下ESC键时退出d
            exit()
...
class SceneLike(Listener):  # 场景的类，管理障碍物、角色、地图背景的描绘、刷新等
    ...
    def __init__(self, player):
        ...
        self.window_scale = (800, 600)  # 显示窗口的大小
        ...
        if event.code == REQUEST_MOVE:  # 监听玩家的移动请求事件
            ...
            for wall in self.walls:  # 遍历所有障碍物
                if wall.rect.colliderect(target_rect):  # 调用pygame的方法检测碰撞
                    can_move = 0  # 有碰撞则不能移动
                    break  # 有碰撞时不用再判断后面的了
            if self.map_range[0] - target_rect.x < 60 or target_rect.x < 0:
                can_move = 0
            if self.map_range[1] - target_rect.y < 60 or target_rect.y < 0:
                can_move = 0
