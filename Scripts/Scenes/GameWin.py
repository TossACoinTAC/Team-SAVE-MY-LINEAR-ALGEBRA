...
class ReplayButton(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.scaled_images = {
            1: pygame.transform.smoothscale(self.image, (self.rect.width * 2, self.rect.height * 2)),
            1.2: pygame.transform.smoothscale(self.image, (self.rect.width * 2.4, self.rect.height * 2.4))}
    ...
    def update(self):
        ...
        if (self.rect.left <= mouse_x <= self.rect.right and self.rect.top <= mouse_y <= self.rect.bottom):
            ...
            if pygame.mouse.get_pressed()[0]:
                pygame.event.post(pygame.event.Event(Events.TO_MAIN))
        else:
